...
; When the index is already in SI (here forced there, but naturally the case when
; it is live across a call), the add+load folds into a single [bx+si] load.
; CHECK-LABEL: folded:
; CHECK: mov ax, [bx + si]
; CHECK-NOT: add bx,
define i16 @folded(i16* %a) {
